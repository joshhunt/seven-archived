...
/**
	Represents the different data that a Membership *can* have attached to it.
	
	 It does not guarantee that a membership *will* have such data attached to it.
	*/
export enum BungieMembershipRoleMappings {
  None = 0,
  BungieNet = 1,
  Destiny1 = 2,
  Destiny2 = 4,
...
export enum DestinyProgressionRewardItemState {
  ...
  /**
		...
		An item can be earned but still can't be claimed in certain circumstances, like if it's only allowed
		for certain subclasses.
		It also might not be able to be claimed if you already claimed it!
		*/
  ClaimAllowed = 8,
}
...
/**
	A flags enumeration indicating one or more platform Subscriptions that may modify the versions or seasons
	of Destiny to which you have access.
	*/
export enum DestinySubscriptions {
  None = 0,
  StadiaPro = 1,
}
...
export enum DestinyComponentType {
  ...
  /**
		This will get you information about the silver that this profile has on every platform
		 on which it plays.
		
		 You may only request this component for the logged in user's Profile, and will not
		 recieve it if you request it for another Profile.
		*/
  PlatformSilver = 105,
  /**
  ...
  /**
		Sometimes, designers create thousands of reusable plugs and suddenly your response sizes are almost 3MB, and something has to give.
		 ...
		 Request this component if and only if you need to know what plugs *could* be inserted into a socket, and need to know it
		 before "drilling" into the details of an item in your application (for instance, if you're doing some sort of interesting
		 sorting or aggregation based on available plugs.
		
		 When you get this, you will also need to combine it with "Plug Sets" data if you want a full picture of
		 all of the available plugs: this component will only return plugs that have state data that is per-item.  See Plug Sets
		 for available plugs that have Character, Profile, or no state-specific restrictions.
		*/
  ...
  Records = 900,
  /**
		Returns information that Bungie considers to be "Transitory": data that may change too frequently or come from
		a non-authoritative source such that we don't consider the data to be fully trustworthy, but that might prove useful
		for some limited use cases.  We can provide no guarantee of timeliness nor consistency for this data: buyer
		beware with the Transitory component.
		*/
  Transitory = 1000,
  /**
...
export enum DestinyJoinClosedReasons {
  ...
  /**
		...
		Basically, the game can't let you join at this time, but for reasons that aren't
		under the control of this user.
		*/
  InternalReasons = 8,
  /**
...
export enum DestinyUnlockValueUIStyle {
  ...
  /**
		Show the value as a floating-point number. For example: "4.52"
...
/**
	Some Objectives provide perks, generally as part of providing some kind of interesting modifier
	for a Challenge or Quest.  This indicates when the Perk is granted.
	*/
...
/**
	When a Stat (DestinyStatDefinition) is aggregated, this is the rules used for determining
	the level and formula used for aggregation.
...
export enum VendorInteractionType {
  ...
  QuestAccept = 7,
  /**
		Honestly, this doesn't seem consistent to me.  It is used to give you choices in the Cryptarch as well as
		some reward prompts by the Eververse vendor.  I'll have to look into that further at some point.
		*/
  ProgressTab = 8,
  /**
...
/**
	...
	And yeah, RiseOfIron and Comet have their own special highlight states.  Don't ask me,
...
/**
	If progression is earned, this determines whether the progression shows visual effects on
	the character or its item - or neither.
	*/
...
export enum DestinyPresentationScreenStyle {
  ...
  Default = 0,
  /**
		Show sub-items as "category sets".  In-game, you'd see these as a vertical list of
		child presentation nodes - armor sets for example - and the icons of items within those sets
...
export enum TalentNodeStepPropertiesTypes {
  None = 0,
...
export enum SpamReductionLevel {
  None = 0,
  Default = 1,
  Minor = 2,
  Major = 3,
...
export enum DestinyMilestoneType {
  ...
  Daily = 4,
  /**
		Special indicates that the event is not on a daily/weekly cadence, but does occur more than once.
