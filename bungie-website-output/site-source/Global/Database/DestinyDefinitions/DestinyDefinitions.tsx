...
export type DefinitionsFetcherized<T extends DestinyDefinitionType> = {
   ...
   * @param hash
   */
  get(hash: number): DestinyWorldDefinitionsTypeMap[T];
  /** Get all definitions for this type */
  all: DestinyWorldDefinitionsGenerated[T];
};
  ...
  constructor() {
    ...
    this.worker.onmessage = (event) => {
      ...
      if (eventData.name === "definitions") {
        ...
        const fetcherize = (type: string, defs: { [key: string]: any }) => {
          return {
            ...
            all: () => Object.keys(defs).map((k) => defs[k]),
          };
        };
